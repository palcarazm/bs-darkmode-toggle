 ...
 */
function initTestDataAttributes() {
  let elementDiv, testDiv;
  DESCRIPTION.html("Check <code>bs-darkmode-toggle</code> data attributes");
  OPTIONS.forEach((testcase) => {
    elementDiv = $("<div></div>")
      .attr("data-toggle", "bs-darkmode-toggle")
      .attr("data-root", "#" + testcase.code);
    testcase.options.forEach((option) => {
      elementDiv.attr("data-" + option.key, option.value);
    });
    testDiv = TEST_CONTAINER.clone().attr("id", testcase.code);
    testDiv.append(
      $('<div class="row mb-3">').append(COL.clone().append(elementDiv))
    );
    testDiv.append(
      $('<div class="row align-items-center">').append(COL.clone())
    );
    MAIN.append(TEST_TITLE.clone().html("Case " + testcase.name), testDiv);
  });
}
