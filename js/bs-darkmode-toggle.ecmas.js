...
(function () {
  class DarkmodeToggle {
    ...
    constructor(element, options) {
      ...
      if (this.element.getAttribute("data-state")) {
        ...
        else if (this.element.getAttribute("data-state") === "light")
          state = true;
      }
      this.options = {
        ...
        allowCookie:
          this.element.hasAttribute("data-allowCookie") ||
          options.allowCookie ||
    ...
    #init() {
      ...
      this.bsToggle = document.createElement("input");
      this.bsToggle.setAttribute("type", "checkbox");
      this.element.innerHTML = "";
      this.element.appendChild(this.bsToggle);
      ...
      // 2: Add listener
      this.bsToggle.addEventListener("change", (e) => {
    ...
    #update() {
      ...
      // 2: Set Color scheme
      document.querySelectorAll(this.options.root).forEach((ele) => {
        ele.setAttribute(
          this.BS_ATTRIBUTE,
          this.options.state
    ...
    setCookieAuthorization(status) {
      this.options.allowCookie = status;
      if (status) {
        this.#setCookie(
          this.COOKIE_NAME,
          this.options.state
   ...
   */
  Element.prototype.bsDarkmodeToggle = function (options, args) {
    let _bsDarkmodeToggle =
      this._bsDarkmodeToggle || new DarkmodeToggle(this, options);
    ...
    if (options && typeof options === "string") {
      switch (options.toLowerCase()) {
        ...
        case "dark":
          ...
          break;
        case "allowcookie":
          _bsDarkmodeToggle.setCookieAuthorization(true);
          break;
        case "denycookie":
          _bsDarkmodeToggle.setCookieAuthorization(false);
          break;
